...
async function getSongs(folder) {
  currFolder = folder;
  let a = await fetch(`https://gunjankmishra.github.io/spotify/${folder}/`)
...
async function displayAlbums() {
  let a = await fetch("https://gunjankmishra.github.io/spotify/songs/")
  let response = await a.text();
  let div = document.createElement("div");
  div.innerHTML = response;
  ...
  for (let index = 0;
    ...
    if (e.href.includes("/songs")) {
      ...
      let a = await fetch(`https://gunjankmishra.github.io/spotify/songs/${folder}/info.json`)
